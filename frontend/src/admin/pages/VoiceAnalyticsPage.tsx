  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Voice Analytics</h1>
          <p className="mt-1 text-slate-600">Live call metrics, latency, and provider usage</p>
        </div>
        <div className="flex items-center gap-2">
          <span
            className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-medium ${
              monitorConnected ? 'bg-emerald-500/20 text-emerald-300' : 'bg-slate-600/50 text-slate-600'
            }`}
          >
          ...
          <button
            ...
            className="rounded-lg border border-slate-300 bg-slate-200 px-3 py-1.5 text-sm text-slate-800 hover:bg-slate-300"
          >
            Refresh
          </button>
        </div>
      ...
      {error && (
        <div className="rounded-lg border border-red-400/50 bg-red-500/10 p-3 text-sm text-red-800">{error}</div>
      )}
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Card className="border-slate-200 bg-slate-50">
          ...
          <CardContent>
            <p className="text-2xl font-bold text-slate-900">{overview?.activeCalls ?? 0}</p>
          </CardContent>
        </Card>
        <Card className="border-slate-200 bg-slate-50">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-slate-600">Total calls today</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-2xl font-bold text-slate-900">{overview?.totalCallsToday ?? 0}</p>
          </CardContent>
        </Card>
        <Card className="border-slate-200 bg-slate-50">
          <CardHeader className="pb-2">
        ...
        <Card className="border-slate-200 bg-slate-50">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-slate-600">Avg latency (STT+LLM+TTS)</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-2xl font-bold text-slate-900">
          ...
          {activeCalls.length === 0 ? (
            ...
            <ul className="space-y-2">
              {activeCalls.map((c) => (
                <li
                  key={c.callId}
                  className="flex items-center justify-between rounded-lg border border-slate-300 bg-slate-200/50 px-3 py-2"
                >
                  <span className="font-mono text-sm text-slate-700">{c.callId.slice(0, 8)}…</span>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card className="border-slate-200 bg-slate-50">
          <CardHeader>
            <CardTitle className="text-slate-900">Calls per hour</CardTitle>
        ...
        <Card className="border-slate-200 bg-slate-50">
          <CardHeader>
            <CardTitle className="text-slate-900">Avg latency trend</CardTitle>
      ...
      <Card className="border-slate-200 bg-slate-50">
        <CardHeader>
          <CardTitle className="text-slate-900">Recent calls</CardTitle>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {recentCalls.map((c) => (
                  <tr
                    key={c.callId}
                    className="cursor-pointer border-b border-slate-200 hover:bg-slate-200/50"
                    onClick={() => openInspector(c)}
                  >
                    <td className="py-2 pr-4 font-mono text-slate-700">{c.callId.slice(0, 12)}…</td>
                    <td className="py-2 pr-4 text-slate-600">{c.agentId?.slice(0, 8) ?? '—'}…</td>
                    <td className="py-2 pr-4 text-slate-600">{formatTime(c.startedAt)}</td>
                    <td className="py-2 pr-4 text-slate-600">{formatDuration(c.durationMs)}</td>
                    <td className="py-2 pr-4 text-slate-600">{formatMs(c.sttLatencyMs)}</td>
                    <td className="py-2 pr-4 text-slate-600">{formatMs(c.llmFirstTokenMs)}</td>
                    <td className="py-2 pr-4 text-slate-600">{formatMs(c.ttsFirstAudioMs)}</td>
                    <td className="py-2 pr-4 text-slate-600">{c.interruptions ?? 0}</td>
      ...
      {inspectorCall && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={() => setInspectorCall(null)}>
          <div
            className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-xl border border-slate-300 bg-slate-100 p-6 shadow-xl"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-start justify-between gap-4">
              <div>
                <h2 className="text-lg font-semibold text-slate-900">Call details</h2>
                <p className="mt-1 font-mono text-sm text-slate-600">{inspectorCall.callId}</p>
              </div>
              <button
                type="button"
                onClick={() => setInspectorCall(null)}
                className="rounded p-1 text-slate-600 hover:bg-slate-300 hover:text-slate-900"
              >
                <X className="h-5 w-5" />
              </button>
            </div>
            <dl className="mt-4 grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
              ...
              <dd className="font-mono text-slate-700">{inspectorCall.agentId || '—'}</dd>
              <dt className="text-slate-500">Started</dt>
              <dd className="text-slate-700">{new Date(inspectorCall.startedAt).toLocaleString()}</dd>
              <dt className="text-slate-500">Duration</dt>
              <dd className="text-slate-700">{formatDuration(inspectorCall.durationMs)}</dd>
              <dt className="text-slate-500 flex items-center gap-1"><Mic className="h-3.5 w-3.5" /> STT latency</dt>
              <dd className="text-slate-700">{formatMs(inspectorCall.sttLatencyMs)}</dd>
              <dt className="text-slate-500 flex items-center gap-1"><Cpu className="h-3.5 w-3.5" /> LLM first token</dt>
              <dd className="text-slate-700">{formatMs(inspectorCall.llmFirstTokenMs)}</dd>
              <dt className="text-slate-500 flex items-center gap-1"><Volume2 className="h-3.5 w-3.5" /> TTS first audio</dt>
              <dd className="text-slate-700">{formatMs(inspectorCall.ttsFirstAudioMs)}</dd>
              <dt className="text-slate-500">Interruptions</dt>
              <dd className="text-slate-700">{inspectorCall.interruptions ?? 0}</dd>
            ...
            {inspectorOutcome && (
              <div className="mt-6 rounded-lg border border-slate-300 bg-slate-200/50 p-3">
                <h3 className="flex items-center gap-2 text-sm font-medium text-slate-600 mb-2">
                ...
                <div className="space-y-1 text-sm">
                  ...
                  <div className="flex justify-between gap-2">
                    ...
                    <span className="text-slate-800">{Math.round(inspectorOutcome.confidence * 100)}%</span>
                  </div>
                  <p className="text-slate-700 pt-2 border-t border-slate-300 mt-2">{inspectorOutcome.summary}</p>
                </div>
              </div>
            )}
              ...
              <div className="mt-6 rounded-lg border border-slate-300 bg-slate-200/50 p-3">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div>
                    ...
                    <p className="text-slate-700 mt-0.5">{inspectorEvaluation.strengths}</p>
                  </div>
                  <div>
                    <span className="text-slate-500">Improvements:</span>
                    <p className="text-slate-700 mt-0.5">{inspectorEvaluation.improvements}</p>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="mt-6">
              ...
              <div className="mt-2 max-h-48 overflow-y-auto rounded-lg border border-slate-300 bg-slate-200/50 p-3">
