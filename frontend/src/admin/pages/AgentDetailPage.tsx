  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-slate-50 overflow-hidden">
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className="w-full flex items-center justify-between px-5 py-4 text-left text-sm font-semibold text-slate-800 hover:bg-slate-200/50 transition-colors"
      >
        <span>{title}</span>
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900">
      <div className="max-w-7xl mx-auto px-6 py-6 space-y-6">
        {/* Breadcrumb */}
        <Link to="/admin/agents" className="text-sm text-slate-600 hover:text-slate-800 transition-colors">
          ← Agents
        </Link>
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex flex-wrap items-center gap-4">
            <input
              ...
              className="text-2xl font-semibold bg-transparent border-b border-slate-300 hover:border-slate-600 focus:border-emerald-500 focus:outline-none px-0 py-1 min-w-[200px]"
          ...
          <div className="flex items-center gap-3">
            <Link
              to="/admin/web-call"
              state={{ agentId }}
              className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-slate-900 text-sm font-medium transition-colors"
            >
            ...
            <button
              ...
              className="px-5 py-2 rounded-lg bg-slate-700 hover:bg-slate-600 disabled:opacity-50 text-sm font-semibold text-slate-900 transition-colors"
            >
        ...
        {error && (
          <div className="p-4 rounded-xl border border-red-400/50 bg-red-500/10 text-red-800 text-sm">
            {error}
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-6">
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-5">
              <h2 className="text-sm font-semibold text-slate-800 mb-4">System prompt</h2>
              <textarea
                value={systemPrompt}
                onChange={(e) => setSystemPrompt(e.target.value)}
                className="w-full min-h-[420px] max-h-[500px] rounded-lg bg-slate-50 border border-slate-200 px-4 py-3 text-sm font-mono text-slate-800 placeholder-slate-500 focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/30 focus:outline-none resize-y"
            ...
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-5">
              <h2 className="text-sm font-semibold text-slate-800 mb-4">Preview with variables</h2>
              <div className="min-h-[120px] max-h-[200px] overflow-y-auto rounded-lg bg-slate-50 border border-slate-200 p-4 text-sm font-mono text-slate-700 whitespace-pre-wrap">
          ...
          <div className="space-y-4">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full rounded-lg bg-slate-50 border border-slate-200 px-3 py-2 text-sm text-slate-800 placeholder-slate-500 focus:border-emerald-500/50 focus:outline-none"
                  />
                </div>
                {agent?.agentType === 'PIPELINE' && (
                  <>
                    <div className="border-t border-slate-200 pt-4">
                      <h3 className="text-xs font-semibold text-slate-600 mb-2">Speech to Text</h3>
                      <div className="space-y-2">
                        <div>
                          ...
                          <select
                            ...
                            className="w-full rounded-lg bg-slate-50 border border-slate-200 px-3 py-2 text-sm text-slate-800 focus:border-emerald-500/50 focus:outline-none"
                          >
                            {STT_PROVIDERS.map((p) => (
                    ...
                    <div className="border-t border-slate-200 pt-4">
                      <h3 className="text-xs font-semibold text-slate-600 mb-2">Language Model</h3>
                      <div className="space-y-2">
                        <div>
                        ...
                        <div>
                          ...
                          <input
                            ...
                            className="w-full h-2 rounded-lg bg-slate-200 accent-emerald-500"
                          />
                        </div>
                      </div>
                    </div>
                    <div className="border-t border-slate-200 pt-4">
                      <h3 className="text-xs font-semibold text-slate-600 mb-2">Text to Speech</h3>
                      <div className="space-y-2">
                        <div>
                ...
                {agent?.agentType === 'V2V' && (
                  <div className="border-t border-slate-200 pt-4">
                    <h3 className="text-xs font-semibold text-slate-600 mb-2">V2V Realtime (provider, model, voice)</h3>
                ...
                <div>
                  <label className="block text-xs font-medium text-slate-600 mb-1">Max call duration (seconds)</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="block text-xs font-medium text-slate-600 mb-1">Interruption</label>
                  <select
            ...
            <Section title="Custom variables" defaultOpen={true}>
              ...
              <div className="space-y-2">
                {Object.entries(customVariables).map(([k, v]) => (
                  <div key={k} className="flex items-center gap-2 rounded-lg bg-slate-50/80 px-3 py-2 border border-slate-200">
                    <span className="text-sm font-mono text-slate-700 flex-1 truncate">{k}</span>
                    <span className="text-sm text-slate-600 truncate max-w-[120px]">{v}</span>
                    <button
                      type="button"
                      onClick={() => removeVariable(k)}
                      className="p-1 rounded text-slate-600 hover:text-red-400 hover:bg-slate-200"
              ...
              <div className="flex gap-2 mt-3">
                <input
                  ...
                  placeholder="Key"
                  className="flex-1 rounded-lg bg-slate-50 border border-slate-200 px-3 py-2 text-sm placeholder-slate-500 focus:border-emerald-500/50 focus:outline-none"
                />
                <input
                  type="text"
                  value={variableValue}
                ...
                <button
                  type="button"
                  onClick={addVariable}
                  className="inline-flex items-center gap-1 px-3 py-2 rounded-lg bg-slate-700 hover:bg-slate-600 text-sm font-medium text-slate-800"
                >
            ...
            <Section title="Knowledge base" defaultOpen={true}>
              <div>
                <label className="block text-xs font-medium text-slate-600 mb-1">RAG knowledge base</label>
                <select
            ...
            <Section title="Intro message" defaultOpen={false}>
              ...
              <textarea
                ...
                rows={3}
                className="w-full rounded-lg bg-slate-50 border border-slate-200 px-3 py-2 text-sm text-slate-800 placeholder-slate-500 focus:border-emerald-500/50 focus:outline-none resize-none"
              />
            </Section>
            ...
            <Section title="Tools" defaultOpen={true}>
              ...
              <div className="max-h-40 overflow-y-auto space-y-2">
                {allTools.length === 0 ? (
                  ...
                  allTools.map((t) => (
                    <label key={t.id} className="flex items-center gap-2 cursor-pointer rounded-lg px-2 py-1.5 hover:bg-slate-200/50">
                      <input
                        type="checkbox"
              ...
              {allTools.length > 0 && (
                <button
                  ...
                  className="mt-3 w-full py-2 rounded-lg bg-slate-700 hover:bg-slate-600 text-sm font-medium text-slate-800 disabled:opacity-50"
                >
            ...
            <Section title="Prompt Versions" defaultOpen={true}>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-slate-200 bg-slate-100">
                        <th className="text-left py-2 px-3 text-slate-600 font-medium">Version</th>
                        <th className="text-left py-2 px-3 text-slate-600 font-medium">Traffic</th>
                        <th className="text-left py-2 px-3 text-slate-600 font-medium">Conversion</th>
                        <th className="text-left py-2 px-3 text-slate-600 font-medium">Avg Score</th>
                        <th className="text-left py-2 px-3 text-slate-600 font-medium">Active</th>
                      </tr>
                    </thead>
                    <tbody>
                      {promptVersions.map((v) => {
                        ...
                        return (
                          <tr key={v.id} className="border-b border-slate-200/80 hover:bg-slate-200/30">
                            <td className="py-2 px-3 text-slate-800 font-mono">v{v.version}</td>
                            <td className="py-2 px-3 text-slate-700">{v.trafficShare}%</td>
                            <td className="py-2 px-3 text-slate-700">{perf?.conversionRate != null ? `${perf.conversionRate}%` : '—'}</td>
                            <td className="py-2 px-3 text-slate-700">{perf?.avgScore != null ? Math.round(perf.avgScore) : '—'}</td>
                            <td className="py-2 px-3">
                              <button
                                ...
                                className={`inline-flex items-center rounded px-2 py-0.5 text-xs font-medium ${v.isActive ? 'bg-emerald-600/80 text-slate-900' : 'bg-slate-700 text-slate-600'}`}
                              >
                                {v.isActive ? 'On' : 'Off'}
                              </button>
                            </td>
              ...
              <button
                ...
                className="mt-3 w-full py-2 rounded-lg bg-slate-700 hover:bg-slate-600 disabled:opacity-50 text-sm font-medium text-slate-800"
              >
            ...
            <Section title="AI Prompt Suggestions" defaultOpen={true}>
              ...
              {promptOptimizations.length === 0 ? (
                ...
                <ul className="space-y-3 max-h-48 overflow-y-auto">
                  {promptOptimizations.map((o) => (
                    <li key={o.id} className="rounded-lg bg-slate-50/80 border border-slate-200 px-3 py-2">
                      <p className="text-sm text-slate-800 whitespace-pre-wrap">{o.suggestion}</p>
              ...
              <button
                ...
                className="mt-3 w-full py-2 rounded-lg bg-emerald-700/80 hover:bg-emerald-600/80 disabled:opacity-50 text-sm font-medium text-slate-900"
              >
